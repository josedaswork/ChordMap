...
interface SongListProps {
  ...
  onTriggerSync: (mode: 'upload' | 'download') => void;
}
...
export default function SongList({
  ...
  const getSectionsCount = (sectionsJson: string): number => {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 self-stretch sm:self-auto justify-end">
          <button
            ...
            className="flex-1 sm:flex-initial p-2.5 border-3 border-black bg-white hover:bg-zinc-150 disabled:opacity-50 text-black cursor-pointer shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center gap-1"
          >
            <CloudUpload className={`w-4 h-4 stroke-[2.5] text-blue-600 ${syncStatus === 'loading' ? 'animate-bounce' : ''}`} />
            <span className="text-[10px] sm:text-xs font-black uppercase tracking-wide">Subir Nube</span>
          </button>
          ...
          >
            ...
            <span className="text-[10px] sm:text-xs font-black uppercase tracking-wide">Bajar Nube</span>
          </button>
      ...
      {/* Sync Message Box Banner */}
      {syncStatus !== 'idle' && (
        <div className={`p-3 border-2 border-black text-xs font-bold shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] flex items-center gap-2 ${
          syncStatus === 'loading' ? 'bg-zinc-100 text-zinc-800' :
          syncStatus === 'success' ? 'bg-green-100 text-green-800 border-green-800' :
          'bg-red-100 text-red-800 border-red-800'
        }`}>
          <div className={`w-2.5 h-2.5 rounded-full shrink-0 border border-black ${
            syncStatus === 'loading' ? 'bg-zinc-500 animate-pulse' :
      ...
      {/* Songs Display list or Empty state */}
      {filtered.length === 0 ? (
        <div className="border-4 border-dashed border-black bg-white p-8 md:p-12 text-center rounded-xl max-w-lg mx-auto space-y-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="space-y-1">
            <h3 className="font-black text-black uppercase tracking-wide text-sm md:text-base">
              {search ? "Sin resultados para la búsqueda" : "Guarda tu primera canción"}
            </h3>
            <p className="text-xs text-zinc-500 font-bold leading-relaxed">
              {search 
                ? "No se encontraron títulos o de artistas con ese nombre. Intenta con palabras clave diferentes." 
          ...
          <div className="pt-2">
            {search ? (
              <button
                onClick={() => setSearch('')}
                className="px-4 py-2 bg-black hover:bg-zinc-800 text-white font-black text-xs uppercase tracking-wide cursor-pointer"
              >
                Limpiar búsqueda
              </button>
            ) : (
        ...
        <div className="space-y-3 pb-20">
          ...
          <div className="grid grid-cols-1 gap-3.5">
            {filtered.map((song) => {
              const secCount = getSectionsCount(song.sectionsJson);
              return (
                <div
                  key={song.id}
                ...
                >
                  <div className="space-y-2 flex-1 pr-4">
                    ...
                    {/* Meta labels (Capo, Tuning, Sections) */}
